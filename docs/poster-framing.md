...
## Повторная проверка рамок и карточек — 23 сентября 2026

После замечаний по скриншотам повторно осмотрены все 13 афиш. У акций
«Сын + сын», «Друг + друг», «Отец + сын», «Дед + внук» и перехода из другого
барбершопа действительно отсутствовала верхняя красная сторона рамки.

Теперь `PosterImage` добавляет её отдельным декоративным SVG над исходным
изображением. Координаты, толщина и цвет взяты из боковых полос каждой афиши
и хранятся в `modules/offers/poster-frames.ts`. Дополнительная верхняя полоса
высотой 22–25 исходных пикселей масштабируется вместе с афишей; короткие
боковые соединения затрагивают только пустые начала существующей рамки.
Иллюстрации, волосы, лица и надписи не перекрываются. У остальных восьми
афиш такой вставки нет. Все 13 WebP и бумажная текстура побайтно совпадают
с версией до этой правки; повторного сжатия и генерации не было.

Белые полосы под некоторыми афишами создавались `h-full` и фоном `Card`
при разных пропорциях изображений в одной строке сетки. У карточек убраны
растягивание по высоте, скругление, внешний бордюр и подложка. Каждая
карточка заканчивается точно по нижнему краю своего изображения.
Исходные пропорции и полное содержимое сохраняются; обрезка не используется.
Скругления также убраны у остальных явно скруглённых изображений сайта.

Повторная проверка: 360 / 390 / 768 / 1440 px, все 13 афиш загружаются,
радиус карточек равен нулю, нижние края карточки и изображения совпадают,
горизонтальной прокрутки нет. В браузере нет ошибок JavaScript; dev-режим
показывает существующее предупреждение о lazy-loading первой афиши (LCP).
TypeScript и production-сборка проходят. ESLint не находит ошибок;
остаются два прежних предупреждения о неиспользуемых импортах в `app/page.tsx`.
Для сборки также исправлен прежний обязательный `id` в `BookingLink`:
общая ссылка записи теперь допускает отсутствие выбранной услуги.

## Предыдущая обработка исходных краёв
